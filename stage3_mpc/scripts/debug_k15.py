...
def main():
    ...
    for k in range(17):
        ...
        if k == 14:
            u_prev_plan = mpc.u_prev.copy()   # 第 14 步计划（第 15 步的锚）
        tau, q_des, target_w = realize_step(env, u, snap)
        env.step(tau[None, :], np.ones((1, 4)), q_des[None, :], target_w)
        if k == 14:
            print(f'k=15 开始: z={env.p[0,1]:.4f} vz={env.v[0,1]:+.3f} '
    ...
    print(f'k=15 返回解: obj={fun(x_ret):.3f} '
    ...
    # ---- 不同初值重解同一个 QP（含力矩约束），检查 coneqp 解是否唯一 ----
