...
├── control/     # 论文1：步态时钟、SRB-MPC、摆动腿轨迹
├── rl/          # 论文2：PPO、课程调度、训练主循环
├── analysis/    # 复现图表与指标（静态分析、消融实验）
└── scripts/     # 演示脚本：run_mpc.py / run_rl.py / compare.py
```
...
阶段3 详细报告见 [docs/stage_report_3.md](docs/stage_report_3.md)。
...
.venv/Scripts/python.exe scripts/test_mpc.py            # 4 场景: 平地/墙面行走/墙面保持/天花板
.venv/Scripts/python.exe scripts/debug_mpc.py           # QP解/速度/力实现/z扰动 诊断
.venv/Scripts/python.exe scripts/run_mpc.py             # 键盘: 1/2/3/4=表面, 上下=调速, R=复位
```
